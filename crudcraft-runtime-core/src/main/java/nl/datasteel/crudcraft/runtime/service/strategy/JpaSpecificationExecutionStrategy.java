...
public class JpaSpecificationExecutionStrategy<T> implements QueryExecutionStrategy<T> {
    ...
    /**
     * The repository to execute queries against. This is a stateless, thread-safe proxy provided by
     * Spring Data.
     */
    ...
    @SuppressFBWarnings(
            ...
            justification =
                    "Spring Data repository is a stateless, "
    ...
    /**
     * Executes a paginated query with the given specification.
     *
    ...
    /**
     * Executes a query with the given specification, returning all matching entities.
     *
     * @param spec the JPA specification to apply
    ...
    /**
     * Finds all entities matching the given specification, paginated and projected to the specified
     * type.
     *
     * @param spec the JPA specification to apply
     * @param pageable the pagination information
    ...
    public <R> Page<R> findAll(Specification<T> spec, Pageable pageable, Class<R> projection) {
        return repository.findBy(nonNull(spec), q -> q.as(projection).page(pageable));
    }

    /**
     * Finds all entities matching the given specification, projecting them to the specified type.
     *
     * @param spec the JPA specification to apply
     * @param projection the class of the projection type
     ...
     */
    @Override
    public <R> List<R> findAll(Specification<T> spec, Class<R> projection) {
        return repository.findBy(nonNull(spec), q -> q.as(projection).all());
    }
    ...
    /**
     * Finds a single entity matching the given specification, projecting it to the specified type.
     *
     * @param spec the JPA specification to apply
     * @param projection the class of the projection type
    ...
    public <R> Optional<R> findOne(Specification<T> spec, Class<R> projection) {
        return repository.findBy(nonNull(spec), q -> q.as(projection).first());
    }
    ...
    public boolean exists(Specification<T> spec) {
        return repository.findBy(nonNull(spec), q -> q.first()).isPresent();
    }
    ...
    public long count(Specification<T> spec) {
        return repository.count(nonNull(spec));
    }

    private Specification<T> nonNull(Specification<T> spec) {
