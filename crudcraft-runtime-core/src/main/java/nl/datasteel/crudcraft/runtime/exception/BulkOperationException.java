...
/**
 * Represents partial or complete failures in bulk operations. Contains list of individual
 * item-level exceptions. Results in 207 Multi-Status or 500 by default, handle via
 * ControllerAdvice.
 */
public class BulkOperationException extends CrudCraftRuntimeException {
    ...
    public BulkOperationException(String message, List<Throwable> itemExceptions) {
        super(message);
        List<Throwable> normalized =
                itemExceptions == null
                        ? List.of()
                        : itemExceptions.stream()
                                .map(
                                        throwable ->
                                                throwable != null
                                                        ? throwable
                                                        : new IllegalStateException(
                                                                "Unknown bulk item error"))
                                .toList();
        this.itemExceptions = List.copyOf(normalized);
    }

    /**
     * Returns an immutable list of item exceptions.
     *
     * @return immutable item-level exceptions
     */
