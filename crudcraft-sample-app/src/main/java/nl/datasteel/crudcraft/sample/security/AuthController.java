...
/**
 * Very simple authentication controller exposing a login endpoint. Uses CrudCraft-generated
 * repository and mapper components to authenticate demo users.
 */
@RestController
@RequestMapping("/auth")
public class AuthController {
    ...
    /**
     * Creates the demo authentication controller.
     *
     * @param userRepository repository used to load demo users
     * @param jwtSecret symmetric JWT signing secret
     */
    ...
    /**
     * Authenticates a demo user and issues a short-lived JWT token. The token carries the user's
     * roles and expires in one day, so it can be used for manual testing. Doesn't use
     * PasswordEncoder or other security features because that would make logging in more complex
     * than necessary for a demo.
     */
    @PostMapping("/login")
    ...
    public ResponseEntity<LoginResponse> login(@RequestBody LoginRequest request) {
        return userRepository
                ...
                .map(
                        token -> {
                            String msg =
                                    "Include this token as 'Bearer <token>' in the Authorization"
                                            + " header. It remains valid for 1 day; login again"
                                            + " afterwards.";
