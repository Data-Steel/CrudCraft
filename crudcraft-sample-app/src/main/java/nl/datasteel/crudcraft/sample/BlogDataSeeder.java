...
import nl.datasteel.crudcraft.sample.blog.Comment;
...
import nl.datasteel.crudcraft.sample.blog.content.Article;
import nl.datasteel.crudcraft.sample.blog.content.ContentStatus;
import nl.datasteel.crudcraft.sample.blog.content.DifficultyLevel;
import nl.datasteel.crudcraft.sample.blog.content.Tutorial;
...
/**
 * Seeds the in-memory database with hundreds of sample blog entries. This demonstrates CrudCraft's
 * capabilities with a realistic data volume.
 */
@Component
@SuppressFBWarnings(
        value = {"EI_EXPOSE_REP", "EI_EXPOSE_REP2"},
        justification =
                "Spring injects EntityManager and the seeder keeps it as a final collaborator")
// Sample seeder methods are intentionally private helpers; public API Javadoc is on the class.
...
public class BlogDataSeeder implements CommandLineRunner {
    ...
    private final AuditingRuntimeExtension auditingRuntimeExtension;
    ...
    public BlogDataSeeder(
            EntityManager entityManager,
            AuditingRuntimeExtension auditingRuntimeExtension) {
        this.entityManager = entityManager;
        this.auditingRuntimeExtension = auditingRuntimeExtension;
    }
    ...
    public void run(String... args) {
        ...
        LOG.info("Seeding blog database with sample data...");
    ...
    private List<Category> createCategories() {
        ...
        for (String name : CATEGORY_NAMES) {
            ...
            category.setDescription(
                    "Explore the latest in " + name.toLowerCase() + " and related topics.");
            persist(category);
    ...
    private List<Post> createPosts(
            int count, List<Author> authors, List<Category> categories, List<Tag> tags) {
    ...
    private int createComments(int count, List<Post> posts) {
        ...
        List<Post> publishedPosts =
                posts.stream().filter(p -> p.getStatus() == PostStatus.PUBLISHED).toList();
    ...
    private int createArticles(int count, List<Author> authors) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            article.setTitle(
                    "Article: "
                            + TITLE_TEMPLATES[random.nextInt(TITLE_TEMPLATES.length)].replace(
                                    "%s", CATEGORY_NAMES[random.nextInt(CATEGORY_NAMES.length)]));
            ...
            ContentStatus status =
                    ContentStatus.values()[random.nextInt(ContentStatus.values().length)];
    ...
    private void persist(Object entity) {
        auditingRuntimeExtension.beforeSave(entity);
