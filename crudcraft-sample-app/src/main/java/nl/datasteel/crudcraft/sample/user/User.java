...
import nl.datasteel.crudcraft.runtime.security.policy.AdminOnlySecurityPolicy;
import nl.datasteel.crudcraft.sample.security.RoleType;


/**
 * Application user with roles. Demonstrates field-level security, AdminOnly security policy, and
 * projection fields.
 */
@CrudCrafted(editable = true, secure = true, securityPolicy = AdminOnlySecurityPolicy.class)
@Entity
...
public class User {
    ...
    /**
     * Password hash is write-only. It can be set via the API but will never be returned in
     * responses due to {@link FieldSecurity} with empty readRoles. It is also hard-excluded from
     * entity export via {@link ExportExclude}.
     */
    @Dto
    ...
    /**
     * Internal note demonstrates denied write filtering through generated controllers. Admins may
     * read stored values, but API writes are dropped by {@link FieldSecurity}.
     */
    @Dto
    @FieldSecurity(readRoles = {"ADMIN"}, writeRoles = {}, writePolicy = WritePolicy.SKIP_ON_DENIED)
    @Request
    ...
    @Embedded private AuditableExtension audit = new AuditableExtension();
