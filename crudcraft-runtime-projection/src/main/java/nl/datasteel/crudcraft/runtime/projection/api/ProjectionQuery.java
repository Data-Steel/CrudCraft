 ...
 *
 * @param <T> entity type that is queried
 * @param filter filter criteria used for the query
 * @param pageable paging and sorting configuration
 */
public record ProjectionQuery<T>(FilterCriteria<T> filter, Pageable pageable) {

    /** Canonical constructor that normalizes null filters to a no-op filter. */
