...
public class BookSpecification implements Specification<Book> {
    ...
    private final BookSearchRequest request;

    public BookSpecification(BookSearchRequest request) {
        this.request = new BookSearchRequest(request);
    }
    ...
    public Predicate toPredicate(Root<Book> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (request.getPrice() != null && request.getPriceOp() == SearchOperator.EQUALS) {
            p = logic == SearchLogic.AND ? cb.and(p, root.get("price").in(request.getPrice())) : cb.or(p, root.get("price").in(request.getPrice()));
            hasCriteria = true;
        }
