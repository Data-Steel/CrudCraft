 ...
 */
package demo.golden.include.controller;

import demo.golden.include.dto.response.AuditLogResponseDto;
import demo.golden.include.search.AuditLogSearchRequest;
import demo.golden.include.service.AuditLogService;
...
public class AuditLogController {
    ...
    public AuditLogController(AuditLogService service, ObjectProvider<MeterRegistry> meterRegistry,
    ...
    private void recordOperation(String operation, String outcome, long started) {
        ...
        LOG.info("crudcraft.generated.operation model={} operation={} outcome={} duration_ms={}", "AuditLog", operation, outcome, TimeUnit.NANOSECONDS.toMillis(durationNanos));
        if (meterRegistry != null) {
            Timer.builder("crudcraft.generated.operation").tag("model", "AuditLog").tag("operation", operation).tag("outcome", outcome).register(meterRegistry).record(durationNanos, TimeUnit.NANOSECONDS);
        }
    }
    ...
    @Operation(
            operationId = "auditLogExists",
            summary = "Check if a AuditLog exists",
            description = "Checks whether a AuditLog entity with the given ID exists in the system."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "AuditLog existence check result"),
            @ApiResponse(responseCode = "404", description = "AuditLog with the specified ID was not found")
    })
    ...
    @GetMapping("/count")
    @Operation(
            operationId = "auditLogCount",
            summary = "Count AuditLog entities",
            description = "Counts the total number of AuditLog entities matching the search criteria."
    )
    @ApiResponses(@ApiResponse(responseCode = "200", description = "Count of AuditLog entities"))
    ...
    @Operation(
            ...
            summary = "Search AuditLog entities",
            description = "Searches for AuditLog entities based on the provided search criteria."
    )
    @ApiResponses(@ApiResponse(responseCode = "200", description = "Search results for AuditLog entities"))
    public ResponseEntity<PaginatedResponse<AuditLogResponseDto>> search(
            @ModelAttribute AuditLogSearchRequest searchRequest, Pageable pageable) {
        long _crudcraftStarted = System.nanoTime();
        String _crudcraftOutcome = "success";
        try {
            Pageable clamped = clampPageable(pageable);
            Page<AuditLogResponseDto> page = SearchOperations.search(service, searchRequest, clamped, AuditLogResponseDto.class);
            PaginatedResponse<AuditLogResponseDto> response = new PaginatedResponse<>(
