...
import demo.golden.endpointmatrix.search.SearchOnlyEventSearchRequest;
import demo.golden.endpointmatrix.service.SearchOnlyEventService;
...
@RestController
@RequestMapping("/searchonlyevents")
public class SearchOnlyEventController {
    private static final Logger LOG = LoggerFactory.getLogger(SearchOnlyEventController.class);

    @SuppressFBWarnings("EI_EXPOSE_REP2")
    private final SearchOnlyEventService service;
    ...
    public SearchOnlyEventController(SearchOnlyEventService service,
    ...
    private void recordOperation(String operation, String outcome, long started) {
        ...
        LOG.info("crudcraft.generated.operation model={} operation={} outcome={} duration_ms={}", "SearchOnlyEvent", operation, outcome, TimeUnit.NANOSECONDS.toMillis(durationNanos));
        if (meterRegistry != null) {
            Timer.builder("crudcraft.generated.operation").tag("model", "SearchOnlyEvent").tag("operation", operation).tag("outcome", outcome).register(meterRegistry).record(durationNanos, TimeUnit.NANOSECONDS);
        }
    }

    @GetMapping("/search")
    @Operation(
            operationId = "searchOnlyEventSearch",
            summary = "Search SearchOnlyEvent entities",
            description = "Searches for SearchOnlyEvent entities based on the provided search criteria."
    )
    @ApiResponses(@ApiResponse(responseCode = "200", description = "Search results for SearchOnlyEvent entities"))
    public ResponseEntity<PaginatedResponse<SearchOnlyEventResponseDto>> search(
            @ModelAttribute SearchOnlyEventSearchRequest searchRequest, Pageable pageable) {
        long _crudcraftStarted = System.nanoTime();
        String _crudcraftOutcome = "success";
        try {
            Pageable clamped = clampPageable(pageable);
            Page<SearchOnlyEventResponseDto> page = SearchOperations.search(service, searchRequest, clamped, SearchOnlyEventResponseDto.class);
            PaginatedResponse<SearchOnlyEventResponseDto> response = new PaginatedResponse<>(
