...
import demo.golden.endpointmatrix.service.ValidationOnlyDraftService;
...
@RestController
@RequestMapping("/validationonlydrafts")
public class ValidationOnlyDraftController {
    private static final Logger LOG = LoggerFactory.getLogger(ValidationOnlyDraftController.class);

    @SuppressFBWarnings("EI_EXPOSE_REP2")
    private final ValidationOnlyDraftService service;
    ...
    public ValidationOnlyDraftController(ValidationOnlyDraftService service,
    ...
    private void recordOperation(String operation, String outcome, long started) {
        ...
        LOG.info("crudcraft.generated.operation model={} operation={} outcome={} duration_ms={}", "ValidationOnlyDraft", operation, outcome, TimeUnit.NANOSECONDS.toMillis(durationNanos));
        if (meterRegistry != null) {
            Timer.builder("crudcraft.generated.operation").tag("model", "ValidationOnlyDraft").tag("operation", operation).tag("outcome", outcome).register(meterRegistry).record(durationNanos, TimeUnit.NANOSECONDS);
        }
    }

    @PostMapping("/validate")
    @Operation(
            operationId = "validationOnlyDraftValidate",
            summary = "Validate ValidationOnlyDraft data",
            description = "Validates ValidationOnlyDraft data without persisting it. Returns validation errors if any."
    )
    @ApiResponses(@ApiResponse(responseCode = "200", description = "Validation results"))
    public ResponseEntity<Void> validate(
            @Valid @NotNull @RequestBody ValidationOnlyDraftRequestDto request) {
