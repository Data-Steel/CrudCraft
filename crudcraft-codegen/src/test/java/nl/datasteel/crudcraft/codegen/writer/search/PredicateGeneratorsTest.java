...
if (request.getKey() != null && request.getKeyOp() == nl.datasteel.crudcraft.annotations.SearchOperator.CONTAINS_KEY) {
                p = logic == nl.datasteel.crudcraft.runtime.search.SearchLogic.AND ? cb.and(p, root.joinMap(\"map\", jakarta.persistence.criteria.JoinType.LEFT).key().in(request.getKey())) : cb.or(p, root.joinMap(\"map\", jakarta.persistence.criteria.JoinType.LEFT).key().in(request.getKey()));
                hasCriteria = true;
}
"""),
                Arguments.of(
...
if (request.getValue() != null && request.getValueOp() == nl.datasteel.crudcraft.annotations.SearchOperator.CONTAINS_VALUE) {
                p = logic == nl.datasteel.crudcraft.runtime.search.SearchLogic.AND ? cb.and(p, root.joinMap(\"map\", jakarta.persistence.criteria.JoinType.LEFT).value().in(request.getValue())) : cb.or(p, root.joinMap(\"map\", jakarta.persistence.criteria.JoinType.LEFT).value().in(request.getValue()));
                hasCriteria = true;
}
"""));
    }
    ...
    void generatesExpectedCode(SearchOperator op, String prop, String path, String expected) {
        ...
        assertTrue(
                matchesExpected(op, expected, actual),
                () ->
                        "Generated code didn't match for "
                                + op
