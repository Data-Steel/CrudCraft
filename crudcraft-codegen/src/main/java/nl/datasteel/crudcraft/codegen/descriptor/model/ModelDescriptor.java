...
public class ModelDescriptor {
    ...
    /**
     * Flags indicating various properties of the model, such as whether it is editable or
     * embeddable.
     */
    ...
    /**
     * Constructs a ModelDescriptor with the provided identity, flags, endpoints, and security
     * options.
     *
    ...
    /**
     * Returns the package name of the model, which is used for organizing code and avoiding naming
     * conflicts.
     *
     * @return package name
     */
    ...
    /**
     * Returns the base package of the model, which is used for generating code. This is typically
     * the package where the model's classes are located.
     *
    ...
    /**
     * Returns whether the model is a CrudCraft entity, meaning it is part of the CrudCraft
     * framework.
     *
     ...
     *
     * @return configured row scopes
     */
    public List<RowScope> getRowScopes() {
        return security.getRowScopes();
    }

    /**
     * Returns resolved endpoint expressions from annotation-first RBAC.
     *
     * @return endpoint expression map
     */
    public Map<CrudEndpoint, String> getEndpointExpressions() {
        return security.getEndpointExpressions();
    }

    /**
     * True when annotation-first RBAC expressions are available.
     *
     * @return {@code true} when endpoint expressions are configured
     */
    public boolean hasEndpointExpressions() {
        return security.hasEndpointExpressions();
    }

    /**
     * Returns whether any writable field (included in the request DTO) in this model is annotated
     * with {@code @Lob}.
     *
     * @return true if at least one request-writable field is a LOB, false otherwise
     */
    public boolean hasLobFields() {
        return identity.getFields().stream().anyMatch(fd -> fd.isLob() && fd.inRequest());
    }
    ...
    public List<FieldDescriptor> getLobFields() {
        return identity.getFields().stream().filter(FieldDescriptor::isLob).toList();
    }

    /**
     * Returns LOB fields that are included in the request DTO. Used by controller generation to
     * create multipart file parameters.
     *
     * @return list of writable LOB field descriptors
     */
    public List<FieldDescriptor> getRequestLobFields() {
        return identity.getFields().stream().filter(fd -> fd.isLob() && fd.inRequest()).toList();
    }

    /**
     * Returns LOB fields that are included in the response DTO. Used by mapper generation to
     * force-load lazy LOBs.
     *
     * @return list of response LOB field descriptors
     */
    public List<FieldDescriptor> getResponseLobFields() {
        return identity.getFields().stream().filter(fd -> fd.isLob() && fd.inDto()).toList();
    }
    ...
    /**
     * Computes a hash code for the ModelDescriptor based on its identity, flags, endpoints, and
     * security options.
     *
