...
public class BulkUpsertEndpoint implements EndpointSpecProvider {
    ...
    public EndpointSpec create(ModelDescriptor modelDescriptor) {
        ...
        return new EndpointSpec(
                ...
                md -> AnnotationSpec.builder(EndpointSupport.POST_MAPPING)
                        .addMember("value", "$S", "/batch/upsert").build(),
                ...
                List.of(md -> ParameterSpec.builder(
                                ParameterizedTypeName.get(EndpointSupport.COLLECTION, ClassName.get(dtoReqPkg, dtoReq)),
                                "requests")
                        .addAnnotation(EndpointSupport.REQUEST_BODY)
                        .build()),
                (mb, md) -> mb.addCode(
                        "requests.forEach($T::filterWrite);\n" +
                                "$T dtos = service.upsertAll(requests).stream()\n" +
                                "        .map($T::filterRead)\n" +
                                "        .toList();\n" +
                                "return $T.ok(dtos);\n",
