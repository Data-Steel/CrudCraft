...
/**
 * Generates a compile-time “RelationshipMeta” class with static fix(entity) and clear(entity)
 * methods that re-establish or clear bidirectional links in memory via cached
 * java.lang.reflect.Field references.
 */
public class RelationshipMetaGenerator implements Generator {

    /** Creates a relationship metadata generator. */
    public RelationshipMetaGenerator() {
        // Constructor without any parameters stays empty
    }

    /**
     * A map of relationship types to their corresponding handlers. This allows for easy retrieval
     * of the appropriate handler based on the relationship type defined in the model descriptor.
     */
    ...
    /**
     * Generates the RelationshipMeta class for the given model descriptor. This class contains
     * static methods to fix and clear bidirectional relationships.
     *
    ...
    public List<JavaFile> generate(ModelDescriptor md, WriteContext ctx) {
        ...
        ctx.env()
                ...
                .printMessage(
                        ...
                        "Generating RelationshipMeta for "
                                + md.getName()
                                + " in package "
                                + md.getPackageName()
                                + ".meta");
        ...
        JavaFile javaFile =
                JavaPoetUtils.javaFile(metaPkg, cls.build())
    ...
    /**
     * Filters the fields of the given model descriptor to include only those relationships for
     * which a {@link RelationshipHandler} is available.
     *
     * <p>Unidirectional relations such as {@code MANY_TO_ONE} do not require any bidirectional
     * fix/clear logic, so they are excluded to prevent generation of empty try/catch blocks and
     * unused reflection caches.
     *
     * @param md The ModelDescriptor containing metadata about the model.
    ...
    private List<FieldDescriptor> relationshipFields(ModelDescriptor md) {
        return md.getFields().stream().filter(fd -> handlers.containsKey(fd.getRelType())).toList();
    }

    /**
     * Creates the skeleton of the RelationshipMeta class. This includes the class name, package,
        ...
        return TypeSpec.classBuilder(metaName)
                .addJavadoc(
                        ModelStrictHeader.header(
                                md.getName(), metaPkg, this.getClass().getSimpleName()))
                .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                .addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE).build());
    }

    /**
     * Adds static field caches for each relationship field in the given model descriptor. Each
     * field is stored as a static final java.lang.reflect.Field reference. If the relationship is
     * bidirectional, an additional field for the inverse relationship is also created.
     *
    ...
    private void addFieldCaches(
        ...
        for (FieldDescriptor fd : relFields) {
            ...
            if (fd.getRelType().isBidirectional()) {
                String invProp =
                        fd.getMappedBy() != null && !fd.getMappedBy().isBlank()
                                ? fd.getMappedBy()
        ...
        for (FieldDescriptor fd : relFields) {
            ...
            init.addStatement("$L = $T.class.getDeclaredField($S)", fn, entityType, fd.getName());
            ...
            if (fd.getRelType().isBidirectional()) {
                ...
                init.addStatement(
                        "$L = $T.class.getDeclaredField($S)", invFieldName, targetType, invProp);
    ...
    /**
     * Adds a static method to fix bidirectional relationships for the given entity type. This
     * method iterates over all relationship fields and fixes them using the appropriate handler.
     *
     * @param cls The TypeSpec.Builder for the class being generated.
     * @param md The ModelDescriptor containing metadata about the model.
    ...
    private void addFixMethod(
            ...
            ClassName entityType,
            List<FieldDescriptor> relFields) {
        MethodSpec.Builder fix =
                MethodSpec.methodBuilder("fix")
                        .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                        .addParameter(entityType, "entity");

        if (relFields.isEmpty()) {
            fix.beginControlFlow("if (entity == null)")
                    .addStatement("throw new $T($S)", IllegalArgumentException.class, "entity")
                    .endControlFlow();
    ...
    /**
     * Adds a static method to clear bidirectional relationships for the given entity type. This
     * method iterates over all relationship fields and clears them using the appropriate handler.
     *
     * @param cls The TypeSpec.Builder for the class being generated.
     * @param md The ModelDescriptor containing metadata about the model.
    ...
    private void addClearMethod(
            ...
            ClassName entityType,
            List<FieldDescriptor> relFields) {
        MethodSpec.Builder clear =
                MethodSpec.methodBuilder("clear")
                        .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                        .addParameter(entityType, "entity");

        if (relFields.isEmpty()) {
            clear.beginControlFlow("if (entity == null)")
                    .addStatement("throw new $T($S)", IllegalArgumentException.class, "entity")
                    .endControlFlow();
