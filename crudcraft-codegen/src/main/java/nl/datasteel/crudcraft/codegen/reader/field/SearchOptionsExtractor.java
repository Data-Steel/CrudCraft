...
public class SearchOptionsExtractor implements FieldPartExtractor<SearchOptions> {
    ...
    /** Creates the search options extractor. */
    ...
    public SearchOptions extract(VariableElement field, ProcessingEnvironment env) {
        ...
        List<SearchOperator> operators =
                isSearchable ? Arrays.asList(annotation.operators()) : List.of();
        ...
        FieldPartExtractor.log(
                ...
                field,
                String.format(
                        "Extracting SearchOptions → searchable = %s, "
                                + "operators = %s, depth = %d",
    ...
    private static List<SearchOperator> getDefaultOperatorsFor(
            TypeMirror type, boolean isRelationship) {
    ...
    /**
     * Checks if the field is a JPA relationship (OneToOne, OneToMany, ManyToOne, ManyToMany, or
     * Embedded).
     */
    ...
    private static boolean isNumeric(String t) {
        ...
        for (String suffix : NUMERIC_SUFFIXES) {
    ...
    private static boolean isDateTime(String t) {
        return t.equals("java.time.LocalDate")
                || t.equals("java.time.LocalDateTime")
                || t.equals("java.time.Instant")
                || t.equals("java.time.OffsetDateTime")
                || t.endsWith(" LocalDate")
                || t.endsWith(" LocalDateTime")
                || t.endsWith(" Instant")
                || t.endsWith(" OffsetDateTime");
    }
    ...
    private static boolean isCollection(String t) {
        return t.startsWith("java.util.") && CollectionTypes.containsCollectionType(t);
    }

    /** Checks if the type is a Map. */
    private static boolean isMap(String t) {
        return t.startsWith(CollectionTypes.JAVA_UTIL_MAP);
    }

    // Operator groups

    /** Default operators for String fields. */
    private static final List<SearchOperator> DEFAULT_STRING_OPERATORS =
            List.of(
    ...
    private static final List<SearchOperator> DEFAULT_NUMERIC_OPERATORS =
            List.of(
                    SearchOperator.EQUALS,
                    SearchOperator.GT,
                    SearchOperator.LT,
    ...
    private static final List<SearchOperator> DEFAULT_BOOLEAN_OPERATORS =
            List.of(SearchOperator.EQUALS, SearchOperator.NOT_EQUALS);

    /** Default operators for UUID fields. */
    private static final List<SearchOperator> DEFAULT_UUID_OPERATORS =
            List.of(
    ...
    private static final List<SearchOperator> DEFAULT_DATETIME_OPERATORS =
            List.of(
                    SearchOperator.EQUALS,
    ...
    private static final List<SearchOperator> DEFAULT_ENUM_OPERATORS =
            List.of(
                    SearchOperator.EQUALS, SearchOperator.IN,
                    SearchOperator.NOT_EQUALS, SearchOperator.NOT_IN);

    /** Default operators for collection fields. */
    private static final List<SearchOperator> DEFAULT_COLLECTION_OPERATORS =
            List.of(
                    SearchOperator.CONTAINS,
     ...
     */
    private static final List<SearchOperator> DEFAULT_RELATIONSHIP_OPERATORS =
            List.of(SearchOperator.EQUALS);

    /** Default operators for map fields. */
    private static final List<SearchOperator> DEFAULT_MAP_OPERATORS =
            List.of(
    ...
    private static final List<SearchOperator> DEFAULT_FALLBACK_OPERATORS =
            List.of(SearchOperator.EQUALS);
    ...
    private static final List<String> NUMERIC_SUFFIXES =
            List.of(
                    " int",
                    " Integer",
                    " long",
