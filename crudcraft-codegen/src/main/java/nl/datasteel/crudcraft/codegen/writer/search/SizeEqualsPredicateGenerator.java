...
public class SizeEqualsPredicateGenerator
     ...
     * has a size equal to a specified value.
     *
     * @param f the search field containing the property and operator information
     * @return a CodeBlock representing the predicate logic for the size equals condition
     */
    @Override
    public CodeBlock generate(SearchField f) {
        ...
        return CodeBlock.builder()
                ...
                .addStatement("p = logic == $T.AND ? cb.and(p, cb.equal(cb.size($L), request.get$L())) : cb.or(p, cb.equal(cb.size($L), request.get$L()))",
