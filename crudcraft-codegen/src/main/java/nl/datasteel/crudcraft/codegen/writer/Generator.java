...
/**
 * Base contract for all writer components that turn a {@link ModelDescriptor} into one or more
 * {@link JavaFile}s.
 */
public interface Generator {
    ...
    /**
     * Indicates whether this generator should only be applied to models that are marked as
     * CrudCraft entities. Generators returning {@code true} are executed only when the model
     * descriptor reports {@link ModelDescriptor#isCrudCraftEntity()} as {@code true}.
     *
    ...
    /**
     * Determines the execution order within the generator group (basic or CRUD). Lower values run
     * first.
     *
    ...
    /**
     * Performs basic sanity checks on the descriptor before code generation. Logs a diagnostic
    ...
    static boolean isValidModelDescriptor(ModelDescriptor model, WriteContext ctx) {
        if (model == null) {
            ctx.env()
                    .getMessager()
                    .printMessage(
                            Diagnostic.Kind.ERROR, "Model descriptor is null; skipping generation");
            return false;
        }
        if (model.getName() == null
                || model.getName().isBlank()
                || model.getPackageName() == null
                || model.getPackageName().isBlank()) {
            ctx.env()
                    .getMessager()
                    .printMessage(
                            Diagnostic.Kind.ERROR, "Model descriptor has missing name or package");
            return false;
        }
