...
public class BulkCreateEndpoint implements EndpointSpecProvider {
    ...
    public EndpointSpec create(ModelDescriptor modelDescriptor) {
        ...
        return new EndpointSpec(
                ...
                List.of(md -> ParameterSpec.builder(
                                ParameterizedTypeName.get(EndpointSupport.LIST, ClassName.get(dtoReqPkg, dtoReq)),
                                "requests")
                ...
                (mb, md) -> mb.addCode(
                        "requests.forEach($T::filterWrite);\n" +
                                "$T dtos = service.createAll(requests).stream()\n" +
                                "        .map($T::filterRead)\n" +
                                "        .toList();\n" +
                                "return $T.status(201).body(dtos);\n",
