...
public final class TypeUtils {
    ...
    /**
     * Unwraps the generic type from a TypeMirror if applicable, if not returns string name of the
     * fieldType.
     *
     * @param fieldType the TypeMirror to unwrap
     * @param messager messager used for warning output
    ...
    public static String unwrapGeneric(TypeMirror fieldType, Messager messager) {
        ...
        return unwrapDeclaredType(declaredType, fieldType, messager);
    }

    private static String unwrapDeclaredType(
            DeclaredType declaredType, TypeMirror fieldType, Messager messager) {
        try {
            var typeArguments = declaredType.getTypeArguments();
            if (typeArguments.isEmpty()) {
                return fieldType.toString();
            }
            TypeMirror typeArg = typeArguments.getFirst();
            return typeArg.toString();
        } catch (Exception e) {
            return warnAndReturnTypeName(fieldType, messager);
        }
    }

    private static String warnAndReturnTypeName(TypeMirror fieldType, Messager messager) {
        messager.printMessage(
                Diagnostic.Kind.WARNING, "Failed to unwrap generic from type: " + fieldType);
        return fieldType.toString();
    }
