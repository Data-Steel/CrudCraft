...
public class OneToManyHandler implements RelationshipHandler {
    /** Creates a one-to-many relationship handler. */
    public OneToManyHandler() {}

    /**
     * Adds the fix logic for a one-to-many or many-to-many relationship field. This method updates
     * the inverse relationship (either a single field or collection).
     *
    ...
    public void addFix(
        ...
        if (fieldDescriptor.getRelType() == RelationshipType.ONE_TO_MANY) {
            ...
            addUpdateWithCollectionInverse(
                    modelDescriptor, fieldDescriptor, fix, entityType, "add");
        }
    }

    /**
     * Adds the clear logic for a one-to-many or many-to-many relationship field. This method clears
     * the inverse relationship (either sets to null or removes from collection).
     *
     * @param modelDescriptor the model descriptor
     * @param fieldDescriptor the field descriptor
    ...
    public void addClear(
        ...
        if (fieldDescriptor.getRelType() == RelationshipType.ONE_TO_MANY) {
            ...
            addUpdateWithCollectionInverse(
                    modelDescriptor, fieldDescriptor, clear, entityType, "remove");
        }
    }

    /**
     * Writes the loop & inverse-maintenance logic for ONE_TO_MANY relationships where the inverse
     * side is a single reference (ManyToOne).
     *
     * @param rhsExpr either "entity" for fix or "null" for clear
     */
    private void addUpdateWithSingleInverse(
            ModelDescriptor modelDescriptor,
            FieldDescriptor fieldDescriptor,
        ...
        TypeName childCollection =
                ParameterizedTypeName.get(ClassName.get(Collection.class), child);
        ...
        method.addStatement(
                "@SuppressWarnings(\"unchecked\") $T $L = ($T)$L.get(entity)",
                childCollection,
                childrenVar,
                childCollection,
                fieldName);
    ...
    /**
     * Writes the loop & inverse-maintenance logic for MANY_TO_MANY relationships where the inverse
     * side is also a collection.
     *
     * @param operationOnInverse either "add" or "remove"
     */
    private void addUpdateWithCollectionInverse(
            ModelDescriptor modelDescriptor,
            FieldDescriptor fieldDescriptor,
            MethodSpec.Builder method,
            ClassName entityType,
        ...
        TypeName childCollection =
                ParameterizedTypeName.get(ClassName.get(Collection.class), child);
        TypeName inverseCollection =
                ParameterizedTypeName.get(ClassName.get(Collection.class), entityType);
        ...
        method.beginControlFlow("if ($L != null)", childrenVar)
                .beginControlFlow("for ($T $L : $L)", child, childVar, childrenVar)
                .addStatement(
                        "@SuppressWarnings(\"unchecked\") $T $L = ($T)$L.get($L)",
                        inverseCollection,
                        invVar,
                        inverseCollection,
                        inverseRelationshipFieldName,
                        childVar)
