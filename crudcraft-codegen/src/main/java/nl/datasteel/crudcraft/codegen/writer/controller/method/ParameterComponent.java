...
public class ParameterComponent implements ControllerMethodComponent {
    ...
    public void apply(ControllerMethodContext ctx) {
        ctx.spec().params().stream()
                .map(fn -> {
                    ParameterSpec p = fn.apply(ctx.model());
                    return Objects.requireNonNull(p, "Parameter generator returned null");
                })
                .forEach(ctx.builder()::addParameter);
    }
}
