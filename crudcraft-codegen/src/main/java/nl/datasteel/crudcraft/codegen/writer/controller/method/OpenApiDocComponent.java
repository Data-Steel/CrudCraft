...
/**
 * Adds OpenAPI documentation annotations (@Operation, @ApiResponse, @Parameter) to controller
 * methods.
 */
public class OpenApiDocComponent implements ControllerMethodComponent {
    /** Creates the OpenAPI documentation component. */
    public OpenApiDocComponent() {}
    ...
    private void addApiResponses(
            ControllerMethodContext ctx, CrudEndpoint endpoint, String modelName) {
        ...
        responsesBuilder.addMember(
                "value", "$L", createSuccessResponse(endpoint, modelName, apiResponse));
        ...
        if (requiresIdParameter(endpoint)) {
            responsesBuilder.addMember(
                    ...
                    createErrorResponse(
                            "404",
                            String.format("%s with the specified ID was not found", modelName),
                            apiResponse));
        }

        if (isModifyingEndpoint(endpoint)) {
            responsesBuilder.addMember(
                    "value", "$L", createErrorResponse("400", "Invalid request data", apiResponse));
        }
        if (isBulkEndpoint(endpoint)) {
            responsesBuilder.addMember(
                    ...
                    createErrorResponse(
                            "207",
                            "Bulk request completed with one or more per-item failures",
                            apiResponse));
        }
    ...
    private CodeBlock createSuccessResponse(
            CrudEndpoint endpoint, String modelName, ClassName apiResponse) {
    ...
    private CodeBlock createErrorResponse(String code, String description, ClassName apiResponse) {
        return CodeBlock.builder()
                .add("@$T(responseCode = $S, description = $S)", apiResponse, code, description)
                .build();
    }
    ...
    private String getOperationDescription(CrudEndpoint endpoint, String modelName) {
        return switch (endpoint) {
            case POST ->
                    "Creates a new "
                            + modelName
                            + " entity with the provided data. "
                            + "Returns the created entity with generated ID.";
            case BULK_CREATE ->
                    "Creates multiple "
                            + modelName
                            + " entities in a single request. "
                            + "Returns per-item success and failure details.";
            case PUT ->
                    "Updates an existing "
                            + modelName
            ...
            case BULK_UPDATE ->
                    "Updates multiple "
                            + modelName
                            + " entities in a single request. "
                            + "Each entity must include its ID. Returns per-item success and "
                            + "failure details.";
            case BULK_PATCH ->
                    "Partially updates multiple "
                            + modelName
                            + " entities in a single request. "
                            + "Each entity must include its ID. Returns per-item success and "
                            + "failure details.";
            ...
            case BULK_DELETE ->
                    "Permanently deletes multiple "
                            + modelName
                            + " entities by their IDs. Returns per-item success and failure "
                            + "details.";
            case GET_ONE -> "Retrieves a single " + modelName + " entity by its unique identifier.";
            case GET_ALL ->
                    "Retrieves all "
                            + modelName
                            + " entities with support for pagination, "
                            + "sorting, and filtering via search parameters.";
            case GET_ALL_REF ->
                    "Retrieves lightweight references to "
                            + modelName
                            + " entities "
            ...
            case FIND_BY_IDS ->
                    "Retrieves multiple "
                            + modelName
                            + " entities by their IDs in a single request.";
            case EXISTS ->
                    "Checks whether a "
                            + modelName
                            + " entity with the given ID exists in the system.";
            case COUNT ->
    ...
    /**
     * Gets the operation ID based on the endpoint type and model name. Pattern:
     * {modelName}{EndpointType} (e.g., "postExists", "authorCreate")
     */
    ...
    private boolean isModifyingEndpoint(CrudEndpoint endpoint) {
        return endpoint == CrudEndpoint.POST
                ...
                || endpoint == CrudEndpoint.BULK_DELETE;
    }

    /** Checks if the endpoint returns a bulk result envelope. */
    private boolean isBulkEndpoint(CrudEndpoint endpoint) {
        return endpoint == CrudEndpoint.BULK_CREATE
                || endpoint == CrudEndpoint.BULK_UPDATE
                || endpoint == CrudEndpoint.BULK_PATCH
                || endpoint == CrudEndpoint.BULK_UPSERT
                || endpoint == CrudEndpoint.BULK_DELETE;
