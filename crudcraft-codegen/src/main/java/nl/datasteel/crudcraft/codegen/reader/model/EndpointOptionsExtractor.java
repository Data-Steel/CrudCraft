...
/**
 * Singleton extractor for {@link EndpointOptions}. Extracts endpoint-related options from a model
 * annotated with {@link CrudCrafted}.
 */
...
public class EndpointOptionsExtractor implements ModelPartExtractor<EndpointOptions> {
    ...
    /** Creates the endpoint options extractor. */
    ...
    public EndpointOptions extract(TypeElement cls, ProcessingEnvironment env) {
        ...
        CrudTemplate template = (annotation != null) ? annotation.template() : CrudTemplate.FULL;

        CrudEndpoint[] omit =
                (annotation != null) ? annotation.omitEndpoints() : new CrudEndpoint[0];

        CrudEndpoint[] include =
                (annotation != null) ? annotation.includeEndpoints() : new CrudEndpoint[0];
