...
public final class JavaPoetUtils {
    ...
    private static final String GENERATED_INDENT = "    ";
    ...
    /**
     * Creates a {@link JavaFile.Builder} using CrudCraft's generated-source formatting policy.
     *
     * @param packageName generated package name
     * @param type top-level generated type
     * @return configured JavaFile builder
     */
    public static JavaFile.Builder javaFile(String packageName, TypeSpec type) {
        return JavaFile.builder(packageName, type)
                .skipJavaLangImports(true)
                .indent(GENERATED_INDENT);
    }
    ...
    /**
     * Adds a private field with standard getter and setter methods. Optionally applies validation,
     * enum schema and security annotations.
     *
    ...
    public static void addFieldWithAccessors(
        ...
        for (AnnotationSpec validation : validations) {
            fs.addAnnotation(validation);
        }
        if (fieldSecurity != null) {
            fs.addAnnotation(fieldSecurity);
        }
        builder.addField(fs.build()).addMethod(getter(name, type)).addMethod(setter(name, type));
    }

    /**
     * Adds a private final field with a standard getter method. Optionally applies validation, enum
     * schema and security annotations.
     *
     * @param builder the {@link TypeSpec.Builder} to add to
     * @param name field name
     * @param type field type
    ...
    public static void addFieldWithGetter(
        ...
        builder.addField(fs.build()).addMethod(getter(name, type));
    }
    ...
    private static MethodSpec getter(String name, TypeName type) {
        ...
        MethodSpec.Builder mb =
                MethodSpec.methodBuilder(pre + up).addModifiers(Modifier.PUBLIC).returns(type);
        if (isSet(type)) {
            mb.addStatement(
                    "return this.$N == null ? null : $T.copyOf(this.$N)",
                    name,
    ...
    private static MethodSpec setter(String name, TypeName type) {
        String up = Character.toUpperCase(name.charAt(0)) + name.substring(1);
        MethodSpec.Builder mb =
                MethodSpec.methodBuilder("set" + up)
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(type, name);
        if (isSet(type)) {
            mb.addStatement(
                    "this.$N = $N == null ? null : new $T<>($N)",
                    name,
                    name,
    ...
    private static boolean isSet(TypeName type) {
        return hasRawType(type, ClassName.get(Set.class));
    }

    private static boolean isList(TypeName type) {
        return hasRawType(type, ClassName.get(List.class));
    }

    private static boolean hasRawType(TypeName type, ClassName rawType) {
        if (!(type instanceof ParameterizedTypeName pt)) {
            return false;
        }
        return pt.rawType().equals(rawType);
    }
}
