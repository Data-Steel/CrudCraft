...
/**
 * Utility class for generating getter and setter methods for fields in search request DTOs. This
 * class provides methods to create method specifications for accessing and modifying private fields
...
public final class SearchAccessorUtil {
    ...
    public static MethodSpec getter(String name, TypeName type) {
        ...
        MethodSpec.Builder mb =
                MethodSpec.methodBuilder("get" + up).addModifiers(Modifier.PUBLIC).returns(type);
        if (isSet(type)) {
            mb.addStatement(
    ...
    public static MethodSpec setter(String name, TypeName type) {
        ...
        if (isSet(type)) {
            mb.addStatement(
                    ...
                    name,
                    name,
                    ClassName.get(java.util.HashSet.class),
                    name);
        } else if (isList(type)) {
            mb.addStatement(
                    "this.$N = $N == null ? null : new $T<>($N)",
