...
/**
 * Singleton extractor for extracting relationships from fields annotated with JPA annotations. This
 * class implements the FieldPartExtractor interface for Relationship type.
 */
...
public class RelationshipExtractor implements FieldPartExtractor<Relationship> {
    ...
    /** Creates the relationship extractor. */
    ...
    public Relationship extract(VariableElement field, ProcessingEnvironment env) {
        ...
        if (fieldType == null) {
            ...
            return new Relationship(
                    RelationshipType.NONE, "", "java.lang.Object", false, isEmbedded, false);
        }
        ...
        try {
            ...
            if (otm != null && !otm.mappedBy().isBlank()) {
                ...
                return build(
                        ...
                        otm.mappedBy(),
                        TypeUtils.unwrapGeneric(fieldType, messager),
                        env,
                        isEmbedded);
            }
            ...
            if (mtm != null) {
                ...
                return build(
                        ...
                        mtm.mappedBy(),
                        extractTarget(field, fieldType, messager),
                        env,
                        isEmbedded);
            }
            ...
            if (mto != null) {
                FieldPartExtractor.log(messager, Kind.NOTE, field, "Detected @ManyToOne");
                return build(
                        RelationshipType.MANY_TO_ONE, "", fieldType.toString(), env, isEmbedded);
            }
            ...
            return new Relationship(
                    RelationshipType.NONE, "", fieldType.toString(), false, isEmbedded, false);
        } catch (Exception e) {
            messager.printMessage(
                    Kind.ERROR, "Error extracting relationship from field: " + e.getMessage());
            return new Relationship(
                    RelationshipType.NONE, "", "java.lang.Object", false, isEmbedded, false);
        }
    }
    ...
    private static String extractTarget(
            VariableElement field, TypeMirror fieldType, Messager messager) {
        try {
